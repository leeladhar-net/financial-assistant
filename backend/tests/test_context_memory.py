...
@pytest.mark.asyncio
async def test_context_memory_intent_resolution():
    # Scenario 1: User says "what about its price?" with context "AAPL"
    r1 = await IntentRouter.classify_intent("what about its price?", last_symbol="AAPL")
    ...
    r2 = await IntentRouter.classify_intent("any news?", last_symbol="TSLA")
    ...
    r3 = await IntentRouter.classify_intent("should I buy it?", last_symbol="NVDA")
    ...
    r4 = await IntentRouter.classify_intent("Compare it with MSFT", last_symbol="GOOGL")
